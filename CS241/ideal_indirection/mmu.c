...
void ptHelper(page_table_entry* ptEntry, mmu* this);
void pdHelper(page_directory_entry* pdEntry, mmu* this);
...
void mmu_read_from_virtual_address(mmu *this, addr32 virtual_address,
    ...
    if ( !address_in_segmentations(this->segmentations[pid], virtual_address) || !(find_segment(this->segmentations[pid], virtual_address)->permissions) || !READ) {
        mmu_raise_segmentation_fault(this);
        return;
    }
    addr32 ptVirtualAddress = 0xFFFFF000 & virtual_address;
    page_table_entry* ptEntry = tlb_get_pte(&(this->tlb), ptVirtualAddress);//check on this
 
    if (!ptEntry) {
        mmu_tlb_miss(this);
 
        page_directory_entry* pdEntry = &(this->page_directories[pid])->entries[virtual_address >> (VIRTUAL_ADDR_SPACE - NUM_OFFSET_BITS)];
 
        if (!pdEntry->present) {
            pdHelper(pdEntry, this);
        }
        ptEntry = &(((page_table* )get_system_pointer_from_pde(pdEntry))->entries[(virtual_address & 0x003FF000) >> NUM_OFFSET_BITS]);
    }
 
    if (!ptEntry->present) {
        ptHelper(ptEntry, this);
    }
 
    ptEntry->accessed = 1;
    memcpy(buffer, ((void*)get_system_pointer_from_pte(ptEntry)) + (virtual_address & 0xFFF), num_bytes);
    tlb_add_pte(&this->tlb, ptVirtualAddress, ptEntry);
}
...
void mmu_write_to_virtual_address(mmu *this, addr32 virtual_address, size_t pid,
    ...
    if ( !address_in_segmentations(this->segmentations[pid], virtual_address) || !(find_segment(this->segmentations[pid], virtual_address)->permissions) || !WRITE) {
        mmu_raise_segmentation_fault(this);
        return;
    }
    addr32 ptbVirtualAddress = 0xFFFFF000 & virtual_address;
    page_table_entry* ptEntry = tlb_get_pte(&this->tlb, ptbVirtualAddress);
    ...
    ptEntry->dirty = 1;
    ptEntry->accessed = 1;
    memcpy(((void*)get_system_pointer_from_pte(ptEntry)) + (virtual_address & 0xFFF), buffer, num_bytes);
    tlb_add_pte(&this->tlb, ptbVirtualAddress, ptEntry);
    return;
}

void ptHelper(page_table_entry* ptEntry, mmu* this) {
    ptEntry->base_addr = (ask_kernel_for_frame(ptEntry) >> NUM_OFFSET_BITS);
    ptEntry->user_supervisor = true;
    ptEntry->read_write = true;
    ptEntry->present = true;
    read_page_from_disk((page_table_entry*)ptEntry);
    mmu_raise_page_fault(this);
}

void pdHelper(page_directory_entry* pdEntry, mmu* this){
    pdEntry->base_addr = (ask_kernel_for_frame(NULL) >> NUM_OFFSET_BITS);
    pdEntry->user_supervisor = false;
    pdEntry->read_write = true;
    pdEntry->present = true;
    read_page_from_disk( (page_table_entry*) pdEntry);
    mmu_raise_page_fault(this);
}
