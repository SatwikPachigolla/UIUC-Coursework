...
/*
This mp is about slicing floorplans both vertically and horizontally. Basically the modules are slides out
either horizontally or vertically and this is done until the modules are separated. The first function
basically switches the orientation of the modules. The next one basically changes horizontal cuts to vertical,
while changing vertical cuts to horizontal. The next function is able to swap two modules. The next function
is used to swap the topologies. Basically switches the a an b on the slicing tree. The next function is the
postfix traversal where we traverse the left and right trees. The final function is to get the total resource
which is used to add all of the resources of the modules.

partners: abhaysp2, sap3, ashwind2
*/
...
int is_leaf_node(node_t* ptr) {
  ...
  if(ptr->left == NULL && ptr->right == NULL){
      return 1; //when left of the pointer and right of pointer are NULL return 1
  }
  return 0; // when not the case return 0
}
...
int is_internal_node(node_t* ptr) {
  // TODO:
  if(ptr->left != NULL || ptr->right != NULL){
     return 1; //if one or the other doesn’t equal NULL return 1
  }
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO:
  if(a == b) {
     return 1; //when the values at a and b are equivalent return 1
  }

  if(is_internal_node(a)) {
...
void rotate(node_t* ptr) {
  // TODO:
  int x = ptr->module->w; //storing pointer
  ptr->module->w = ptr->module->h; //moving one to the other
  ptr->module->h = x; //store value is moved the other pointer so the two have been swapped
}
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if(ptr->cutline == V) {
      ptr->cutline = H; //when equal V make it equal H
  }

  else if(ptr->cutline == H) {
      ptr->cutline = V; //when equal H make it equal V
  }
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  a->module = b->module;
  b->module = temp; // swapped the variables
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  struct NODE* m; //temporary variable
  struct NODE*n; //temporary variable
  struct NODE*p; //temporary variable
  m=a->parent->left; //this will be used to check if a is to the left
  n=b->parent->right;// this will be used to check if b is  to the right
  if (m==a){
    a->parent->left=b;// verifying its left
  }
  else{
    a->parent->right=b; //if not left then it has to be right
  }
  if (n==b){
    b->parent->right=a; //same process for b
  }
  else{
    b->parent->left=a;// same process for b
  }
  p=a->parent;// using temp variable to swap parents
  a->parent=b->parent;// moving b's parent into a's parents
  b->parent=p; //here using temp variable original a's parent for b's parent
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  postfix_traversal(ptr->left, nth, expression); //traverse the left
  postfix_traversal(ptr->right, nth, expression); //traverse the right

  if(ptr->module == NULL){
    expression[*nth].cutline = ptr->cutline;
    expression[*nth].module = NULL; //if them module equals NULL
  }
  else{
    expression[*nth].module = ptr->module;
    expression[*nth].cutline = UNDEFINED_CUTLINE; //when module doesn\u2019t equal NULL
  }
  (*nth)++; //incrementing
}
...
{
    if(ptr == NULL){
        return 0; //if the ptr is null return 0
    }
    if(is_leaf_node(ptr) == 1){
      return ptr->module->resource + get_total_resource(ptr->right) + get_total_resource(ptr->left); // if its equal to 1
    }
    else{
      return get_total_resource(ptr->right) + get_total_resource(ptr->left); //if not equal to 1
    }
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  assert(n >= 0 && n < num_modules); // function for n greater than or equal to 0 and less than num_moduels

  node_t * curr = (node_t*)malloc(sizeof(node_t));

  if(n == num_modules -1){
    //create last left leaf node in the tree
    curr->cutline = UNDEFINED_CUTLINE; //undefined
    curr->module = &(modules[n]); //address is in module
    curr->left = NULL; //nothing in left of curr
    curr->right = NULL; //nothing in right of curr
    curr->parent = par; //parent of curr takes par value
    par->left = curr; //if n is equal to num_modules-1

    //create last right leaf node in the traverse
    node_t * right = (node_t*)malloc(sizeof(node_t));
    right->cutline = UNDEFINED_CUTLINE;
    right->module = &(modules[n-1]);
    right->left = NULL; //left of right is nothing
    right->right = NULL; //right of right is nothing
    right->parent = par; //parent of right takes par value
    par->right = right; //right of par takes right value

    return curr; //returns curr
  }

  curr->cutline = V; //cutline will get V
  curr->module = NULL; //module has nothing
  curr->parent = par; //parent of curr takes par value
