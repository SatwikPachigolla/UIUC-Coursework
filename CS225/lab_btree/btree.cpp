...
{
    // std::cout<<*subroot<<std::endl<<std::endl;
    /* TODO Finish this function */
    size_t index = insertion_idx(subroot->elements, key);

    try{
      if(key==subroot->elements[index].key){
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    child->elements.assign(new_left->elements.begin(), mid_elem_itr);
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    if(index<subroot->elements.size() && pair==subroot->elements[index]){
      return;
    }
