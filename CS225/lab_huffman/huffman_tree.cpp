...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for(const Frequency & x : frequencies){
      singleQueue.push(new TreeNode(x));
    }
    ...
    while(mergeQueue.size() + singleQueue.size()>1){
      ...
      int freqnew = one->freq.getFrequency()+two->freq.getFrequency();
      Frequency anew = Frequency(freqnew);
      TreeNode* addition = new TreeNode(anew);
      addition->left = one;
      addition->right = two;
      mergeQueue.push(addition);
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.getNextBit()==1){
      ...
      TreeNode* leafnode = new TreeNode(temp);
      return leafnode;
    }
    else{
