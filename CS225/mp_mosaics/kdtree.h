...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* createKDTree(const vector<Point<Dim>>& curPoints, int curDim);
    Point<Dim> quickSelect(const vector<Point<Dim>>& curPoints, int curDim, int nthSmallest);
    Point<Dim> findMedian(const vector<Point<Dim>>& curPoints, int curDim);
    vector<Point<Dim>> partitionLeft(const vector<Point<Dim>>& curPoints, Point<Dim> median, int curDim);
    vector<Point<Dim>> partitionRight(const vector<Point<Dim>>& curPoints, Point<Dim> median, int curDim);

    Point <Dim> neighborHelper(Point<Dim> currBest, KDTreeNode* curr, const Point<Dim>& target, int curDim) const;

    void clear(KDTreeNode* root);
    typename KDTree<Dim>::KDTreeNode* copy(const KDTreeNode* root);
